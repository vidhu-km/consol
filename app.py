...
REQUIRED_FORECAST_COLS = (COL_TYPE_CURVE, COL_FORECAST_MONTH, COL_FORECAST_YEAR, COL_FORECAST_REVENUE, COL_FORECAST_OPERATING_INCOME, COL_FORECAST_CASH_FLOW)
...
def prepare_forecasts(forecasts: pd.DataFrame, ref_curves: set[str]) -> pd.DataFrame:
    ...
    for src, dst in [(COL_FORECAST_REVENUE, 'total_revenue_dollars'), (COL_FORECAST_OPERATING_INCOME, 'operating_income_dollars'), (COL_FORECAST_CASH_FLOW, 'cash_flow_dollars')]:
...
def build_type_curve_lifetime(forecasts_clean: pd.DataFrame) -> pd.DataFrame:
    agg = forecasts_clean.groupby(COL_TYPE_CURVE).agg(lifetime_revenue_dollars=('total_revenue_dollars', 'sum'), lifetime_operating_income_dollars=('operating_income_dollars', 'sum'), lifetime_cash_flow_dollars=('cash_flow_dollars', 'sum'), forecast_periods=('producing_month', 'count')).reset_index()
    return agg
...
def build_event_forecasts(wells_classified: pd.DataFrame, forecasts_clean: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, row in wells_classified.iterrows():
        ...
        old_curves: list[str] = row['populated_old_curves']
        new_curve: str = row[COL_NEW_CURVE]
        base_cols = ['producing_month', COL_FORECAST_YEAR, COL_FORECAST_MONTH, 'total_revenue_dollars', 'operating_income_dollars', 'cash_flow_dollars']
...
def build_event_annual_forecasts(event_forecasts: pd.DataFrame) -> pd.DataFrame:
    ...
    agg = event_forecasts.groupby(group_cols, as_index=False).agg(total_revenue_dollars=('total_revenue_dollars', 'sum'), operating_income_dollars=('operating_income_dollars', 'sum'), cash_flow_dollars=('cash_flow_dollars', 'sum'))
    return agg
...
def render_event_explorer(econ: pd.DataFrame, wells_classified: pd.DataFrame, indicators_clean: pd.DataFrame, forecasts_clean: pd.DataFrame, event_forecasts: pd.DataFrame, event_annual: pd.DataFrame):
    ...
    if len(ev_fc) > 0:
        ...
        metric_map = {'Revenue': ('total_revenue_dollars', 'cumulative_total_revenue_dollars'), 'Operating Income': ('operating_income_dollars', 'cumulative_operating_income_dollars'), 'Cash Flow': ('cash_flow_dollars', 'cumulative_cash_flow_dollars')}
        ...
        if sel_fc_view == 'Monthly':
            fig = px.line(ev_fc, x='producing_month', y=ev_fc[monthly_col] / 1000000.0, color='series_label', labels={'producing_month': 'Producing Month', 'y': f'{sel_fc_metric} ($MM)', 'color': 'Series'})
            ...
            fig = px.line(ev_fc, x='producing_month', y=ev_fc[cum_col] / 1000000.0, color='series_label')
            ...
            fig = px.bar(ev_ann, x=COL_FORECAST_YEAR, y=ev_ann[monthly_col] / 1000000.0, color='series_label', barmode='group', labels={COL_FORECAST_YEAR: 'Year', 'y': f'{sel_fc_metric} ($MM)', 'color': 'Series'})
